...
Step 2: First, install the necessary packages with `pip install -r requirements.txt`. Run `code/json_to_csv_business.py` and `code/json_to_csv_review.py` at the top of the repo to convert the JSON files into CSV files. This would generate `business.csv` in the `data` folder and seven CSV files for reviews in the `review` sub-folder. 
...
## IV. Running the Analysis 
 To rerun the analysis, run the following codes stored in the code folder: `embedding.py`, `match.py`, and `widget.py`. 
